...
export default function Home() {
  ...
  const requestParseMutation = async (key: string) => fetch('/api/sql-statement-parser', {
    method: "POST",
    body: JSON.stringify({
      filename: key,
    })
  ...
  const fetchData = async (url: string) => {
    try {
      ...
      if (data.sql === '') {
        // likely an error
        setSqlLines('--- Select File ---')
      } else {
        setSqlLines(data.sql);
      }
    } catch (err) {
      console.error(err)
    }
  ...
  useEffect(() => {
    if (selectedFilePos != null) {
      const selectedFile = files[selectedFilePos];
      fetchData(`${selectedFile.url}.json`);
    }
  }, [selectedFilePos, files]);
      ...
      <div className="grid grid-cols-3 w-screen px-4 gap-2">
        <section className="col-span-1 flex flex-col gap-2 overflow-auto">
            {files.map((file, index) => (
              <button key={file.filename} className="rounded-md border-black border-2 px-6 py-3 cursor-pointer text-ellipsis text-wrap break-all" onClick={() => {
